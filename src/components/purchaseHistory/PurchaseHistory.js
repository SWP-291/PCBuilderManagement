        ...
        <div className="container py-5 history">
            <div className="container">
                ...
                <Table striped hover >
                    ...
                    {data && data.map((info) => (
                        <tr key={info.id}>
                            <td><img src={info.pcDTO.image} alt =''/></td>
                            <td><strong>{info.paymentDTO.name}</strong></td>
                            <td>{formatDate(info.orderDate)}</td>
                            <td>{info.paymentDTO.paymentMode}</td>
                            <td style={{color: 'red'}}>{formatNumberWithCommas(info.paymentDTO.amount)} VND</td>
                            <td style={{ color: statusColors[info.statusId] }}>{info.statusId}</td>
                        </tr>
                    ))}
