...
const updateBrand = async (brandId, data) => {
  try {
    const res = await axios.put(`https://localhost:7262/api/Brand/${brandId}`, data);
    ...
    <div className="container py-5 newBrand">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Col md className="contentBrand">
          ...
          <input type="text" id="file" name='logo' value={state.logo} onChange={handleInputChange} />
        ...
        <Row>
            <Col md className="contentBrand" id="check">
              <div>
                ...
                <input type="checkbox" name="status" checked={state.isPublic} onChange={handleInputChange}/>
              </div>
            </Col>
        </Row>
        
        <div className='form-button'>
        <Button type='submit'>{id ? "Update Brand" : "Create Brand"}</Button>
        </div>
