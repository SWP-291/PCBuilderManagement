...
export default function NewPc() {
    ...
    const { name, summary, detail, description, price, discount, templateId, isPublic, designBy, image  } = state;
    ...
    const updatePc = async (pcId, data) => {
      try {
        const res = await axios.put(`https://localhost:7262/api/PC/${pcId}`, data);
    ...
    const validateForm = () => {
        ...
        if (isNaN(templateId) || parseInt(templateId) > 1 || parseInt(templateId) < 0 ||templateId === '') {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Row>
            <Col md className="contentPc" id='abc'>
              ...
              <input type="text" name='summary' value={state.summary} onChange={handleInputChange} />
              {errors.summary_err && <span className='error'>{errors.summary_err}</span>}
            </Col>            
            <Col md className="contentPc" id='abc'>
              <label htmlFor="detail">Detail: </label>
              <input type="text" name='detail' value={state.detail} onChange={handleInputChange} />
              {errors.detail_err && <span className='error'>{errors.detail_err}</span>}
            </Col>
            <Col md className="contentPc" id='abc'>
              <label htmlFor="description">Description: </label>
              <input type="description" name='description' value={state.description} onChange={handleInputChange} />
        ...
        <Row>
            <Col md className="contentPc" id="check">
              <div>
                ...
                <input type="checkbox" name="isPublic" checked={state.isPublic} onChange={handleInputChange}/>
              </div>
            </Col>
            <Col md className="contentPc" id="check">
              <div>
                <label htmlFor="isTemplate">Is Template?</label>
                <input type="checkbox" name="isTemplate" checked={state.isTemplate} onChange={handleInputChange}/>
              </div>
            </Col>
        </Row>
        <Row>
            ...
            <Col md className="contentPc">
              ...
              <input type="number" name='designBy' value={state.designBy} onChange={handleInputChange} />
              {errors.designBy_err && <span className='error'>{errors.designBy_err}</span>}
            </Col>
        </Row>
        <Col md className="contentPc">
